...
export default function RegisterTaskPage() {
  ...
  useEffect(() => {
    apiFetch('/config').then((cfg) => {
      ...
      form.setFieldsValue({
        ...
        five_sim_phone_attempts: cfg.five_sim_phone_attempts || '3',
        five_sim_otp_timeout_seconds: cfg.five_sim_otp_timeout_seconds || '120',
        five_sim_poll_interval_seconds: cfg.five_sim_poll_interval_seconds || '5',
        ...
        hero_sms_phone_attempts: cfg.hero_sms_phone_attempts || '3',
        hero_sms_otp_timeout_seconds: cfg.hero_sms_otp_timeout_seconds || '120',
        hero_sms_poll_interval_seconds: cfg.hero_sms_poll_interval_seconds || '5',
        ...
        zai_mailbox_attempts: cfg.zai_mailbox_attempts || '3',
  ...
  const submit = async () => {
    ...
    const registerExtra = {
      ...
      chatgpt_blocked_email_domains: values.chatgpt_blocked_email_domains,
      grok_blocked_email_domains: values.grok_blocked_email_domains,
      qwen_blocked_email_domains: values.qwen_blocked_email_domains,
      deepseek_blocked_email_domains: values.deepseek_blocked_email_domains,
      ...
      zai_mailbox_attempts: values.zai_mailbox_attempts,
    ...
    <div style={{ maxWidth: 800 }}>
      ...
      <Form form={form} layout="vertical" onFinish={submit} initialValues={{
        ...
        <Card title="基本配置" style={{ marginBottom: 16 }}>
          ...
          {platform === 'zai' && (
            <Form.Item
              name="zai_mailbox_attempts"
              label="Z.ai 邮箱重试次数"
              extra="Z.ai 当前仅支持 mailbox provider 自动分配邮箱并自动收验证链接。"
            >
              <Input placeholder="3" />
            </Form.Item>
          )}
